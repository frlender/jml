# JML 
<span style="font-size:15px;font-weight:bold">J</span>avascript-based X<span style="font-size:15px;font-weight:bold">ML</span> generator for robotic models
...
## Docker
If docker is installed on your system, you can easily run the tool by first pulling the docker image from dockerhub:
```bash
docker pull frlender/jml:latest
```
Then you can run the tool on a model description file by mounting the model descriptionfile's directory as a volume. Suppose you have a model description file called `model.js` is in your current directory:
```bash
docker run --rm -v `pwd`:/root/models frlender/jml ./models/model.js
```
A `model.xml` should be generated in the same directory.
